...
namespace db
{
	...
	//访问其它玩家数据
	void do_6000(DBBuffer* buff, DBConnector* db)
	{
		int memid = 0;
		buff->r(memid);
		auto start = std::chrono::steady_clock::now();
		...
		//1、定义查询字段
		std::string sss = "HMGET user_data:" + std::to_string(memid)
		+ " memid curhp maxhp speed nick pos_x pos_y pos_z rot_x rot_y rot_z";
		auto redis = db->GetRedisConnector();
		int err = redis->RedisCommand(sss.c_str());
		if (err != 0)
		{
			printf("redis 6000 failed err...\n");
		}
		else
		{
			int length = 0;
			...
			if (length > 0)
			{
				...
				auto current = std::chrono::steady_clock::now();
				auto duration = duration_cast<milliseconds>(current - start);
				printf("redis 6000 %d-%d-%d nick:%s time:%d 毫秒\n", user.memid, user.curhp, user.maxhp, user.nick, (int)duration.count());
				//从工作线程推送数据到主线程
				auto buf2 = __DBManager->PopPool();
				buf2->b(CMD_6000);
				buf2->s(0);
		...
		//3、redis查询失败
		printf("redis 6000 find failed...find mysql...\n");
		...
		if (err != 0)
		{
			printf("mysql 6000 query err...%d %s\n", err, mysql->GetErrorStr());
			auto buf2 = __DBManager->PopPool();
			buf2->b(CMD_6000);
			//一个错误码
			buf2->s(2);
		...
		auto current = std::chrono::steady_clock::now();
		auto duration = duration_cast<milliseconds>(current - start);
		printf("mysql 6000 read %d-%d-%d nick:%s time:%d 毫秒\n", user.memid, user.curhp, user.maxhp, user.nick, (int)duration.count());
